...
class Evaluator:
    ...
    def evaluate(self, train_df: pd.DataFrame, test_df: pd.DataFrame, movies_df: pd.DataFrame):

        # 2. Create the ground truth map (test items)
        test_user_items = test_df[test_df['Rating'] >= self.rating_threshold].groupby('UserID')['MovieID'].apply(set).to_dict()
        ...
        for model_name, model in self.models.items():
            ...
            for user_id in tqdm(test_user_ids[:30], desc=f"Predicting for {model_name}"):
                ...
                # Calculate metrics
                metrics["precision@k"].append(precision_at_k(recommendations, relevant_items, self.k))
                metrics["recall@k"].append(recall_at_k(recommendations, relevant_items, self.k))
                metrics["ndcg@k"].append(ndcg_at_k(recommendations, relevant_items, self.k))
