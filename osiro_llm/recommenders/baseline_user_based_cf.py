...
class UserCFRecommender(BaseRecommender):
    ...
    def fit(self, ratings_df: pd.DataFrame, movies_df: pd.DataFrame):
        ...
        self.user_item_matrix = pd.pivot_table(ratings_df, values='Rating', index='UserID', columns='MovieID').fillna(0)
        ...
        self.user_similarity = pd.DataFrame(user_similarity_matrix, index=self.user_item_matrix.index, columns=self.user_item_matrix.index)
    ...
    def recommend(self, user_id: int, n: int, all_movie_ids: list):
        ...
        similar_users = self.user_similarity[user_id].sort_values(ascending=False).head(self.k)
