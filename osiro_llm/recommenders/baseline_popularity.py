...
class PopularityRecommender(BaseRecommender):
    ...
    def fit(self, ratings_df: pd.DataFrame, movies_df: pd.DataFrame):
        self.item_popularity = ratings_df.groupby("MovieID").size().sort_values(ascending=False)
    ...
    def recommend(self, user_id: int, n: int, all_movie_ids: list):
        ...
        recommendations = [item for item in self.item_popularity.index if item not in user_rated_movies]
